...
export function useRoom(rtUrl: string, initialRoom?: string) {
  ...
  // Actions
  const createRoom   = () => socketRef.current?.emit("room:create");
  const joinRoom     = (id: string) => { setRoom(id); socketRef.current?.emit("room:join", id); };
